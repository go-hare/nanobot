...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("emoticorebot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "emoticorebot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("emoticorebot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "emoticorebot.channels.matrix.asyncio.create_task", _fake_create_task
    )
