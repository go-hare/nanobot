...
def onboard():
    ...
    console.print("\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/emoticorebot#-chat-apps[/dim]")
...
def _make_provider(config: Config):
    ...
    from emoticorebot.providers.custom_provider import CustomProvider
    ...
    from emoticorebot.providers.registry import find_by_name
    spec = find_by_name(provider_name)
    if not model.startswith("bedrock/") and not (p and p.api_key) and not (spec and spec.is_oauth):
        console.print("[red]Error: No API key configured.[/red]")
        console.print("Set one in ~/.emoticorebot/config.json under providers section")
        raise typer.Exit(1)
...
def gateway(
    ...
    from emoticorebot.heartbeat.service import HeartbeatService
    from emoticorebot.runtime.daemon import RuntimeDaemon
...
def status():
    ...
    if config_path.exists():
        from emoticorebot.providers.registry import PROVIDERS
