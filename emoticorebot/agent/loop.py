...
改造说明（ai.md §3 / §6 / §8）：融合执行流（Fusion Pipeline）
────────────────────────────────────────────────
Sense（信号提取）→ Policy（IQ/EQ配方）→ IQ执行（可选）→ EQ共情/润色 → Compose

对话结束后：
  - PAD 状态更新（emotion_mgr.update_from_conversation）
  - 语义记忆压缩（memory.consolidate，异步后台）
  - 关系/情绪记忆写入（relational + affective，即时）
────────────────────────────────────────────────
"""
...
from emoticorebot.agent.subagent import SubagentManager
from emoticorebot.agent.tools.cron import CronTool
from emoticorebot.agent.tools.filesystem import EditFileTool, ListDirTool, ReadFileTool, WriteFileTool
from emoticorebot.agent.tools.message import MessageTool
from emoticorebot.agent.tools.registry import ToolRegistry
from emoticorebot.agent.tools.shell import ExecTool
from emoticorebot.agent.tools.spawn import SpawnTool
from emoticorebot.agent.tools.web import WebFetchTool, WebSearchTool
...
from emoticorebot.session.manager import Session, SessionManager
from emoticorebot.core.fusion_engine import FusionEngine
...
class AgentLoop:
    ...
    核心流程：
    1. 从 bus 接收消息
    2. 计算融合策略（IQ/EQ 权重与执行顺序）
    3. 执行融合管线（IQ 工具链 + EQ 表达 + 统一合成）
    4. 记忆分层写入（语义/关系/情绪）
    5. 更新 PAD 情绪状态
    6. 发回响应
    """
    ...
    def __init__(
        ...
        fusion_cfg = self.emotion_mgr.drive_config.get("fusion", {})
        self.fusion_engine = FusionEngine(workspace=workspace, fusion_config=fusion_cfg)
    ...
    async def _connect_mcp(self) -> None:
        ...
        from emoticorebot.agent.tools.mcp import connect_mcp_servers
        try:
                ...
                if max_tool_calls is not None:
                    ...
                    if remaining <= 0:
                        ...
                        final_content = clean or (
                            "工具调用预算已用尽，我先给出当前可得结论；如需继续深挖，请允许更多工具调用。"
                        )
                        break
    ...
    def _write_relational_memory(
        ...
        self.fusion_engine.memory.relational.save(
            summary, emotion=emotion_label, importance=importance
        )
        logger.debug("Relational memory written: emotion={}", emotion_label)
    ...
    async def _consolidate_memory(self, session: Session, archive_all: bool = False) -> bool:
        return await MemoryStore(self.workspace).consolidate(
            ...
            cold_store=self.context.cold_memory,   # facts 写入语义记忆层
        )
    ...
    async def run(self) -> None:
        ...
        logger.info("Agent loop started (fusion mode)")
    ...
    def _post_process_turn(self, msg: InboundMessage, final_content: str, emotion_label: str) -> None:
        ...
        if emotion_event:
            ...
            self.fusion_engine.memory.affective.save(
        ...
        self._write_relational_memory(msg.content, final_content, emotion_label)
    ...
    async def _process_message(
        ...
        progress_cb = (on_progress or self._build_progress_callback(msg)) if progress_enabled else None
        emotion_label = self.emotion_mgr.get_emotion_label()
        final_content = await self.fusion_engine.run_turn(
            agent=self,
            msg=msg,
            session=session,
            history=history,
        ...
        self._post_process_turn(msg, final_content, emotion_label)
